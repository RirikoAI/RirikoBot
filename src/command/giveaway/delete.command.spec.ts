...
describe('DeleteCommand', () => {
  ...
  const mockDiscordService = {
    client: {
      ...
      giveawaysManager: {
        giveaways: [],
        delete: jest.fn().mockResolvedValue(undefined), // Ensure delete returns a resolved promise
      },
    },
  };
  ...
  describe('runPrefix', () => {
    it('should delete a giveaway with valid message ID', async () => {
      const mockMessage = {
        guild: mockGuild,
        member: mockGuildMember,
        content: '!giveaway delete 1234567890',
        channel: mockTextChannel,
        reply: jest.fn(),
      } as unknown as DiscordMessage;

      jest.spyOn(command, 'deleteGiveaway').mockImplementation(async () => {});

      await command.runPrefix(mockMessage);

      expect(command.deleteGiveaway).toBeCalled();
    });
  });
  ...
  describe('runSlash', () => {
    it('should delete a giveaway with valid message ID', async () => {
      ...
      expect(command.deleteGiveaway).toHaveBeenCalledWith(
        mockInteraction,
        '1234567890',
      );
    });
  ...
  describe('deleteGiveaway', () => {
    it('should send error message if no giveaway found', async () => {
      ...
      await command.deleteGiveaway(mockMessage, '1234567890');
    ...
    it('should delete the giveaway if found', async () => {
      const mockMessage = {
        guild: mockGuild,
        member: mockGuildMember,
        content: '!giveaway delete 1234567890',
      ...
      expect(
        mockDiscordService.client.giveawaysManager.delete,
      ).toHaveBeenCalledWith('1234567890');
      expect(mockMessage.reply).toHaveBeenCalledWith('Giveaway deleted!');
    });
  });
