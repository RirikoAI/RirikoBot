...
import {
  ...
  logout,
  updateFeature,
} from "@dashboard/api/bot";
import { GuildInfo } from "@dashboard/config/types";
...
export function useFeatureQuery<K extends keyof CustomFeatures>(
  ...
  return useQuery(Keys.features(guild, feature), () =>
    getFeature(guild, feature)
  );
}
      ...
      async onSuccess(_, { enabled }) {
        ...
        await client.setQueryData<GuildInfo>(Keys.guild_info(guild), (prev) => {
          ...
          if (enabled) {
            ...
            return {
              ...prev,
              enabledFeatures: prev.enabledFeatures.filter(
                (f) => f !== feature
              ),
            };
          }
  ...
  return useMutation(
      ...
      updateFeature(options.guild, options.feature, options.options),
    {
      async onSuccess(updated, options) {
        return await client.setQueryData(
          Keys.features(options.guild, options.feature),
          updated
        );
      },
    }
