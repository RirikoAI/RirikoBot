...
import {
  MusicAdapterInterface,
  PlayOptions,
  SearchResult,
} from '../music.adapter.interface';
...
export class LavaSharkAdapter implements MusicAdapterInterface {
  ...
  constructor(discordClient: Client) {
    ...
    discordClient.on('raw', (packet) =>
      this.lavashark.handleVoiceUpdate(packet),
    );
  }
  ...
  async play(
    ...
    console.log(
      `Playing in guild ${guildId} on channel ${voiceChannel.id} with query: ${query}`,
    );
