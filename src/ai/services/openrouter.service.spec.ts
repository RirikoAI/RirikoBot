...
describe('OpenRouterService', () => {
  ...
  beforeEach(() => {
    service = new OpenRouterService(mockApiKey, baseUrl);
    jest.clearAllMocks();
    // Mock TextDecoder
  ...
  describe('pullModel', () => {
    it('should return cloud-based model status', async () => {
      ...
      expect(results).toEqual([
        {
          status: `OpenRouter models are cloud-based. Model ${modelId} is ready to use.`,
        },
      ]);
    });
  });
  ...
  describe('getAvailableModels', () => {
    ...
    it('should return default model on API error', async () => {
      ...
      const models = await service.getAvailableModels();
      expect(models).toEqual(['openai/gpt-4.1-nano']);
    });
