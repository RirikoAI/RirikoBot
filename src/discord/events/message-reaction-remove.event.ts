 ...
 * @description Event handler for the messageReactionRemove event
...
export const MessageReactionRemoveEvent = (
  ...
  const logger = new Logger('MessageReactionRemoveEvent');
  ...
  client.on(Events.MessageReactionRemove, async (reaction: MessageReaction | PartialMessageReaction, user: User | PartialUser) => {
    try {
      ...
      const reactionRole = await reactionRoleService.getReactionRole(messageId, emoji);
      ...
      if (!reactionRoleService.hasPermission(guild)) {
        logger.warn(`Bot doesn't have permission to manage roles in guild ${guild.id}`);
        return;
      }

      // Check if the role is valid
      if (!reactionRoleService.isValidRole(guild, reactionRole.roleId)) {
        logger.warn(`Invalid role ${reactionRole.roleId} in guild ${guild.id}`);
        return;
      }
      ...
      // Remove the role
      const success = await reactionRoleService.removeRole(member, role);
      if (success) {
        logger.log(`Removed role ${role.name} from ${user.tag} in guild ${guild.name}`);
      } else {
        logger.error(`Failed to remove role ${role.name} from ${user.tag} in guild ${guild.name}`);
      }
    } catch (error) {
