...
import {backendPort, publicURL, port} from "helpers/getconfig";
...
function startRirikoBotWorker() {
    ...
    worker_RirikoBot.on("message", (message) => {
        if (message.ready) {
            ...
            cli.log(
                "\n" +
                `✅  Dashboard running at ${publicURL()}:${port()}`.brightGreen
            );
            cli.log(
                `✅  Backend running at ${publicURL()}:${backendPort()}`.brightGreen
            );
            cli.log(
                "\n==============================================================\n"
                    .white
