...
export function registerGeneratePoTokenCommand(program: Command): void {
  program
    ...
    .alias('youtube:token')
    .description(
      'Generate YouTube credentials (PO token, visitor data, and cookies via Chrome or JSDOM)',
    )
    .option('-s, --save', 'Save generated credentials directly to active .env file')
    .option(
      '-c, --chrome',
      'Use Google Chrome / Chromium to harvest session cookies alongside tokens (recommended)',
    )
    ...
    .option(
      '-l, --login',
      'Open interactive browser window to log into YouTube account (implies Chrome)',
    )
    ...
    .action(async (options: GeneratePoTokenOptions) => {
      const useBrowser = Boolean(
        options.chrome || options.firefox || options.login || options.browser,
      );
      ...
      if (useBrowser) {
        ...
        try {
          ...
          console.log(
            `  ${pc.cyan('Mode:')}          ${result.isAuthenticated ? pc.green('Authenticated Account Session') : pc.yellow('Guest Browser Session')}`,
          );
          ...
          if (options.save) {
            ...
            console.log(
              pc.green('💾 Saved YOUTUBE_COOKIE, YOUTUBE_PO_TOKEN, and YOUTUBE_VISITOR_DATA to:'),
            );
            console.log(`  ${pc.cyan(envPath)}\n`);
          } else {
            console.log(
              pc.gray('Tip: Run with --save to persist all 3 variables into your .env file:'),
            );
        ...
        console.log(
          pc.cyan('⚡ Initializing YouTube BotGuard challenge runner in secure JSDOM runtime...'),
        );
        ...
        try {
          ...
          if (options.save) {
            ...
            console.log(
              pc.gray('Tip: Run with --save to automatically persist these into your .env file:'),
            );
