...
export function createFarewellCommand(services: BotServices): Command {
  return {
    ...
    async execute(ctx: CommandContext): Promise<void> {
      if (!ctx.guild) {
        throw new CommandGuildOnlyError('The farewell configuration can only be used within a server.');
      }
      ...
      if (isUpdate) {
        ...
        if (!newData.channelId && newData.isEnabled) {
           await ctx.reply('❌ You must set a channel before enabling farewell messages. Example: `/farewell channel:#goodbye`');
           return;
        }
      ...
      if (!config) {
        await ctx.reply('ℹ️ Farewell messages are not configured for this server. Use `/farewell channel:#goodbye` to set it up.');
        return;
      }
