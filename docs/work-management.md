...
## Explicit integration of completed deliveries

RIR-005 adds a reviewed alternative for reconciling completed work after squash merges. Read [ADR-014](adr/ADR-014-completed-delivery-integration.md) and the current integration handoff. A new estimated repair scope may contain an `integration` record instead of `stack`, naming the actual integration decision, exact pre-repair baseline and ordered completed source heads with optional verified squash receipts. Apply it once with the normal locked `integrate` board command; never manually rewrite state.

Sources remain closed. Each source is validated against its own committed board, paths, immediate base and completion evidence. Extra baseline commits are administrative only; new repair work is restricted to the repair paths. All declared originals and squash receipts must be retained before checkpoint. Historical source-topic movement is resolved by explicit immutable provenance; ordinary stacks remain strict about moved refs. A source-owned file does not automatically become writable by the repair.

CI now checks out and validates the actual PR head rather than a synthetic merge checkout. The frozen closed-parent checker remains unchanged: the new integration repair PR supplies the valid checkpoint and reviewed route. Publication still requires a clean exact-head/base approval, and integration-branch merging remains a separate user decision. No completed scope is silently reopened or counted as new feature work.
